...
# 可执行文件路径（相对脚本位置或绝对路径）
BENCH_CLIENT = "./bench_client_gpu"
BENCH_SERVER = "./bench_server_gpu"

# GPU ID（client/server 都用这个 GPU）
GPU_ID = 0

# 结果输出（用一个新的文件，避免和之前混在一起）
RESULT_CSV = "rdma_gpu_msg_sweep.csv"
PLOT_DIR = Path("plots_gpu_msg_sweep")

# 实验参数
FIXED_WINDOW = 64  # 固定 window
ITERS = 200000  # 每组实验的迭代次数
MSG_LIST = [32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 65536]

# 要测的模式（你可以按需改，比如只测 write）
MODES = ["write", "send"]
...
def run_client(mode: str, msg: int, iters: int, window: int):
    """运行 bench_client_gpu 并解析 Mops / GiB/s。
    如果 bench_client_gpu 返回非 0，则返回 None。
    """
    cmd = [
        BENCH_CLIENT,
        SERVER_IP,
...
def ask_start_server(mode: str, msg: int, iters: int):
    """提示你在 server 上启动 bench_server_gpu, 并等待回车。"""
    if mode == "send":
        srv_cmd = (
            f"{BENCH_SERVER} {PORT} "
        ...
        # read/write 模式不需要预贴 recv WR，server 只是提供 RDMA buffer
        srv_cmd = (
            f"{BENCH_SERVER} {PORT} "
...
def run_msg_sweep():
    ...
    print(f"\n\n===== 固定 window={FIXED_WINDOW}, 扫 message size (GPU, {MODES}) =====")
    results = []
...
def plot_results():
    import pandas as pd  # 再 import 一次无伤大雅
    ...
    if sweep.empty:
        print("没有 msg_sweep_gpu 实验数据，先跑实验再画图。")
        return
...
def main():
    ...
    while True:
        print("\n请选择操作:")
        print("  1) 运行 固定 window 的 GPU msg sweep 实验")
