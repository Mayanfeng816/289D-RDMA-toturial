...
The frarework could tune message size, iteration count, and in-flight window/recv depth to see how performance scales. 
...
### Server API
```
./bench_server <port> [--mode read|send] [--msg N] [--iters N] [--recv-depth N]
```
- `--mode`: `read` exposes a buffer for client RDMA READ; `send` preposts receives to accept SENDs.
...
### Client API
```
./bench_client <server_ip> <port> [--mode read|send] [--msg N] [--iters N] [--window N]
```
- `--mode`: `read` issues one-sided RDMA READs; `send` does two-sided SENDs.
